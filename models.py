...
# L2-SP regularizer
class L2SPRegularizer(tf.keras.regularizers.Regularizer):
    weights_based = True

    def __init__(self, initial_weights, regularization_weight=0.01):
    ...
    def __call__(self, x):
        if self.initial_weights is None:
            return 0.0
        return self.regularization_weight * tf.reduce_sum(
            tf.square(x - self.initial_weights)
        )
...
class RouteNet_temporal_delay_l2sp(RouteNet_temporal_delay):
    """
    Expanded version of RouteNet_temporal_delay with support for L2-SP regularization.
    """

    def __init__(self, *args, regularization_weight=0.01, donor_model=None, **kwargs):
        super().__init__(*args, **kwargs)
        assert (
            donor_model is not None
        ), "Donor model must be provided for L2-SP regularization."
        assert (
            regularization_weight > 0
        ), "Regularization weight must be strictly positive."
        self.regularization_weight = regularization_weight

        # Apply L2-SP regularization to all layers with weights
        # GRU Cells used in the Message Passing step
        assert (
            donor_model.flow_state_dim == self.flow_state_dim
        ), "Donor model and new model must have the same flow_state_dim."
        assert (
            donor_model.link_state_dim == self.link_state_dim
        ), "Donor model and new model must have the same link_state_dim."
        assert (
            donor_model.queue_state_dim == self.queue_state_dim
        ), "Donor model and new model must have the same queue_state_dim."
        assert (
            donor_model.max_buffer_types == self.max_buffer_types
        ), "Donor model and new model must have the same max_buffer_types."
        assert (
            donor_model.output_dim == self.output_dim
        ), "Donor model and new model must have the same output_dim."
        donor_layers = {layer.name: layer.get_weights() for layer in donor_model.layers}
        assert all(
            len(weights) for weights in donor_layers.values()
        ), "All donor model layers must have weights. (Model likely not built?)"

        self.flow_update = tf.keras.layers.GRUCell(
            self.flow_state_dim,
            name="FlowUpdate",
            kernel_regularizer=L2SPRegularizer(
                initial_weights=tf.constant(donor_layers["FlowUpdate"][0]),
                regularization_weight=self.regularization_weight,
            ),
            recurrent_regularizer=L2SPRegularizer(
                initial_weights=tf.constant(donor_layers["FlowUpdate"][1]),
                regularization_weight=self.regularization_weight,
            ),
            bias_regularizer=L2SPRegularizer(
                initial_weights=tf.constant(donor_layers["FlowUpdate"][2]),
                regularization_weight=self.regularization_weight,
            ),
        )
        self.link_update = tf.keras.layers.GRUCell(
            ...
            name="LinkUpdate",
            kernel_regularizer=L2SPRegularizer(
                initial_weights=tf.constant(donor_layers["LinkUpdate"][0]),
                regularization_weight=self.regularization_weight,
            ),
            recurrent_regularizer=L2SPRegularizer(
                initial_weights=tf.constant(donor_layers["LinkUpdate"][1]),
                regularization_weight=self.regularization_weight,
            ),
            bias_regularizer=L2SPRegularizer(
                initial_weights=tf.constant(donor_layers["LinkUpdate"][2]),
                regularization_weight=self.regularization_weight,
            ),
        )
        self.queue_update = tf.keras.layers.GRUCell(
            self.queue_state_dim,
            name="QueueUpdate",
            kernel_regularizer=L2SPRegularizer(
                initial_weights=tf.constant(donor_layers["QueueUpdate"][0]),
                regularization_weight=self.regularization_weight,
            ),
            recurrent_regularizer=L2SPRegularizer(
                initial_weights=tf.constant(donor_layers["QueueUpdate"][1]),
                regularization_weight=self.regularization_weight,
            ),
            bias_regularizer=L2SPRegularizer(
                initial_weights=tf.constant(donor_layers["QueueUpdate"][2]),
                regularization_weight=self.regularization_weight,
            ),
        )
        ...
        self.queue_window_update = tf.keras.layers.GRUCell(
            self.queue_state_dim,
            name="QueueWindowUpdate",
            kernel_regularizer=L2SPRegularizer(
                initial_weights=tf.constant(donor_layers["QueueWindowUpdate"][0]),
                regularization_weight=self.regularization_weight,
            ),
            recurrent_regularizer=L2SPRegularizer(
                initial_weights=tf.constant(donor_layers["QueueWindowUpdate"][1]),
                regularization_weight=self.regularization_weight,
            ),
            bias_regularizer=L2SPRegularizer(
                initial_weights=tf.constant(donor_layers["QueueWindowUpdate"][2]),
                regularization_weight=self.regularization_weight,
            ),
        )
        ...
        self.flow_embedding = tf.keras.Sequential(
            [
                ...
                tf.keras.layers.Dense(
                    ...
                    kernel_regularizer=L2SPRegularizer(
                        initial_weights=tf.constant(donor_layers["FlowEmbedding"][0]),
                        regularization_weight=self.regularization_weight,
                    ),
                    bias_regularizer=L2SPRegularizer(
                        initial_weights=tf.constant(donor_layers["FlowEmbedding"][1]),
                        regularization_weight=self.regularization_weight,
                    ),
                ),
                tf.keras.layers.Dense(
                    ...
                    activation=tf.keras.activations.relu,
                    kernel_regularizer=L2SPRegularizer(
                        initial_weights=tf.constant(donor_layers["FlowEmbedding"][2]),
                        regularization_weight=self.regularization_weight,
                    ),
                    bias_regularizer=L2SPRegularizer(
                        initial_weights=tf.constant(donor_layers["FlowEmbedding"][3]),
                        regularization_weight=self.regularization_weight,
                    ),
                ),
            ],
        ...
        self.queue_embedding = tf.keras.Sequential(
            [
                ...
                tf.keras.layers.Dense(
                    ...
                    activation=tf.keras.activations.relu,
                    kernel_regularizer=L2SPRegularizer(
                        initial_weights=tf.constant(donor_layers["QueueEmbedding"][0]),
                        regularization_weight=self.regularization_weight,
                    ),
                    bias_regularizer=L2SPRegularizer(
                        initial_weights=tf.constant(donor_layers["QueueEmbedding"][1]),
                        regularization_weight=self.regularization_weight,
                    ),
                ),
                tf.keras.layers.Dense(
                    ...
                    activation=tf.keras.activations.relu,
                    kernel_regularizer=L2SPRegularizer(
                        initial_weights=tf.constant(donor_layers["QueueEmbedding"][2]),
                        regularization_weight=self.regularization_weight,
                    ),
                    bias_regularizer=L2SPRegularizer(
                        initial_weights=tf.constant(donor_layers["QueueEmbedding"][3]),
                        regularization_weight=self.regularization_weight,
                    ),
                ),
            ],
        ...
        self.link_embedding = tf.keras.Sequential(
            [
                ...
                tf.keras.layers.Dense(
                    ...
                    activation=tf.keras.activations.relu,
                    kernel_regularizer=L2SPRegularizer(
                        initial_weights=tf.constant(donor_layers["LinkEmbedding"][0]),
                        regularization_weight=self.regularization_weight,
                    ),
                    bias_regularizer=L2SPRegularizer(
                        initial_weights=tf.constant(donor_layers["LinkEmbedding"][1]),
                        regularization_weight=self.regularization_weight,
                    ),
                ),
                tf.keras.layers.Dense(
                    ...
                    activation=tf.keras.activations.relu,
                    kernel_regularizer=L2SPRegularizer(
                        initial_weights=tf.constant(donor_layers["LinkEmbedding"][2]),
                        regularization_weight=self.regularization_weight,
                    ),
                    bias_regularizer=L2SPRegularizer(
                        initial_weights=tf.constant(donor_layers["LinkEmbedding"][3]),
                        regularization_weight=self.regularization_weight,
                    ),
                ),
            ],
        ...
        self.readout_path = tf.keras.Sequential(
            [
                tf.keras.layers.Input(shape=(None, self.flow_state_dim)),
                tf.keras.layers.Dense(
                    int(self.link_state_dim / 2),
                    activation=tf.keras.activations.relu,
                    kernel_regularizer=L2SPRegularizer(
                        initial_weights=tf.constant(donor_layers["FlowReadout"][0]),
                        regularization_weight=self.regularization_weight,
                    ),
                    bias_regularizer=L2SPRegularizer(
                        initial_weights=tf.constant(donor_layers["FlowReadout"][1]),
                        regularization_weight=self.regularization_weight,
                    ),
                ),
                tf.keras.layers.Dense(
                    int(self.flow_state_dim / 2),
                    activation=tf.keras.activations.relu,
                    kernel_regularizer=L2SPRegularizer(
                        initial_weights=tf.constant(donor_layers["FlowReadout"][2]),
                        regularization_weight=self.regularization_weight,
                    ),
                    bias_regularizer=L2SPRegularizer(
                        initial_weights=tf.constant(donor_layers["FlowReadout"][3]),
                        regularization_weight=self.regularization_weight,
                    ),
                ),
                tf.keras.layers.Dense(self.output_dim),
...
class RouteNet_temporal_delay_with_encodings(RouteNet_temporal_delay):
    """
    Expanded version of RouteNet_temporal_delay that returns node encodings for
    GTOT-Tuning.
    """
    ...
    def call(self, inputs):
        # Initialize result matrix
        total_results = tf.zeros((0, self.output_dim))
        all_encodings = tf.zeros((0, self.flow_state_dim))
        ...
        loop_invariants = [
            ...
            (all_encodings, tf.TensorShape([None, self.flow_state_dim])),
        ]
        for curr_seg in range(inputs["seg_num"]):
            tf.autograph.experimental.set_loop_options(shape_invariants=loop_invariants)
            ...
            ###################
            # Combine encodings
            all_encodings = tf.concat(
                [all_encodings, flow_state, link_state, queue_state], axis=0
            )
            # Queue and node state update
            queue_state, _ = self.queue_window_update(
                tf.concat([initial_queue_state, previous_queue_state], axis=1),
        ...
        ##################
        if self.log:
            total_results = tf.math.log(total_results)

        return total_results, all_encodings
